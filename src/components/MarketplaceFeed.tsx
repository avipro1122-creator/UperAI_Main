...
export default function MarketplaceFeed({
  ...
  return (
    <section id="marketplace-section" className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      ...
      {/* ── UNIFIED FILTER & SEARCH BAR (Responsive Glassmorphic Container) ─────────── */}
      <div className="relative md:sticky md:top-16 z-20 mb-8 sm:mb-10 p-3.5 sm:p-5 rounded-2xl bg-zinc-950/95 border border-white/10 backdrop-blur-xl shadow-2xl space-y-3 sm:space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 items-center">
          ...
          <div className="lg:col-span-5 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-9 py-2 sm:py-2.5 bg-zinc-900/90 border border-zinc-800 rounded-xl text-xs sm:text-sm font-medium text-white placeholder-zinc-500 focus:outline-none focus:border-lime-400/60 focus:ring-2 focus:ring-lime-400/20 transition-all shadow-inner"
            />
            {searchQuery && (
              <button
          ...
          {/* Niche Pills (7 Cols) - Horizontally scrollable on mobile */}
          <div className="lg:col-span-7 flex items-center gap-1.5 overflow-x-auto no-scrollbar py-0.5 -mx-1 px-1 flex-nowrap lg:flex-wrap">
            {[
              { id: 'all', label: 'All' },
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 sm:px-3.5 sm:py-2 rounded-xl text-xs font-bold transition-all duration-200 shrink-0 whitespace-nowrap ${
                    isActive
        ...
        <div className="pt-2.5 sm:pt-3 border-t border-zinc-800/80 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
          {/* Price Range Slider */}
          <div className="flex items-center gap-2.5 w-full sm:max-w-md bg-zinc-900/80 border border-zinc-800/80 px-3 py-2 rounded-xl">
            <div className="flex items-center gap-1 text-[11px] sm:text-xs font-bold text-zinc-300 shrink-0">
            ...
            <div className="flex-1 flex items-center gap-2 min-w-0">
              <input
                ...
                className="w-full accent-lime-400 cursor-pointer h-1.5 bg-zinc-800 rounded-lg min-w-0"
              />
            </div>
            <span className="text-[11px] sm:text-xs font-extrabold text-lime-400 font-mono shrink-0 px-2 py-0.5 rounded-lg bg-lime-400/10 border border-lime-400/20 whitespace-nowrap">
              {maxRate >= 15000 ? '₹15,000+' : `≤ ₹${maxRate.toLocaleString()}`}
            </span>
          </div>

          {/* Results Counter & Reset Button */}
          <div className="flex items-center justify-between sm:justify-end gap-3 text-[11px] sm:text-xs font-semibold text-zinc-400 px-0.5">
            <span>
            ...
            {hasActiveFilters && (
              <button
                ...
                className="inline-flex items-center gap-1 text-[11px] sm:text-xs font-bold text-lime-400 hover:text-lime-300 transition-colors p-0.5"
              >
