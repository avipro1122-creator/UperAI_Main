...
export default function EditorOnboardingForm({ existing }: { existing?: ExistingData }) {
  ...
  return (
    <div className="space-y-8">
      ...
      {stepError && (
        <div className="p-4 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-400 text-xs font-medium animate-in fade-in space-y-2">
          <p>{stepError}</p>
          {(stepError.toLowerCase().includes('sign in') || !user) && (
            <button
              type="button"
